...
import time
import cma  # Required for CMA-ES

# Configuration identifier
config = 'C4'
print(f"\n===== RUNNING UNCERTAINTY ANALYSIS FOR {config} CONFIGURATION WITH SVR-CMAES =====")
...
try:
    raw_data = pd.read_csv('Downloads/Python files/AlorSetar_C4.csv')
...
# Step 9: Define SVR-CMAES Model and Optimization
print(f"\n===== PROCEEDING TO MODEL TRAINING FOR {config} CONFIGURATION WITH SVR-CMAES =====")

# Define the SVR model and fitness function for CMA-ES
def svm_model_cmaes(params, X_train, X_test, y_train, y_test):
    C = 10 ** params[0]  # Log scale for C
    epsilon = 10 ** params[1]  # Log scale for epsilon
    gamma = 'scale' if params[2] < 0 else 'auto'  # Binary choice for gamma
...
# CMA-ES optimization
def cmaes_optimize(X_train, X_test, y_train, y_test, max_iterations=20):
    x0 = [np.log10(10), np.log10(0.1), 0]  # Initial [log10(C), log10(epsilon), gamma_choice]
    sigma = 0.5  # Initial step size
    objective = lambda params: svm_model_cmaes(params, X_train, X_test, y_train, y_test)
    es = cma.CMAEvolutionStrategy(x0, sigma)
    es.optimize(objective, iterations=max_iterations)
    best_params = es.result.xfavorite
    best_C = 10 ** best_params[0]
    best_epsilon = 10 ** best_params[1]
    best_gamma = 'scale' if best_params[2] < 0 else 'auto'
    return best_C, best_epsilon, best_gamma

# Run CMA-ES optimization
best_C, best_epsilon, best_gamma = cmaes_optimize(X_train, X_test, y_train, y_test)
print(f"Optimized SVR Parameters - C: {best_C:.4f}, Epsilon: {best_epsilon:.4f}, Gamma: {best_gamma}")
...
print("Initial model performance with optimized SVR-CMAES:")
...
for i in range(n_bootstraps):
    ...
    model = SVR(C=best_C, epsilon=best_epsilon, gamma=best_gamma, kernel='rbf')
...
print(f"\n12. FINAL MODEL PERFORMANCE ({config} CONFIGURATION WITH SVR-CMAES)")
print(f"MSE: {mse:.4f}")
print(f"MAE: {mae:.4f}")
...
print(f"\n13. BOOTSTRAP METRIC UNCERTAINTY ({config} CONFIGURATION WITH SVR-CMAES)")
...
plt.ylabel('ET')
plt.legend()
plt.title(f'Predictions with 95% Confidence Intervals - {config} Configuration (Monthly Data) with SVR-CMAES')
plt.grid(True)
plt.ylim(0, 8)
plt.savefig(f'{config}_final_predictions_with_CI_SVR-CMAES.png')
plt.show()

avg_ci_width = np.mean(upper_bound - lower_bound)
rel_ci_width = avg_ci_width / np.mean(y_test_original) * 100

print(f"\n15. UNCERTAINTY SUMMARY ({config} CONFIGURATION WITH SVR-CMAES)")
...
results_df.to_csv(f'{config}_uncertainty_results_SVR-CMAES.csv', index=False)
print(f"Results exported to '{config}_uncertainty_results_SVR-CMAES.csv'")
...
metrics_df.to_csv(f'{config}_performance_metrics_SVR-CMAES.csv', index=False)
print(f"Performance metrics saved to '{config}_performance_metrics_SVR-CMAES.csv'")
